    ...
    for line in source.trim().lines() {
        ...
        let instruction = match instruction {
            ...
            "rcv" => {
                ...
                match a {
                    Argument::Register(_) => Instruction::Rcv(a),
                    Argument::Literal(_) => return Err(String::from("Invalid first argument for rcv instruction.")),
                }
            },
            "jgz" => {
                let b = match b {
...
pub struct Processor {
    ...
    network_rx: VecDeque<i64>,
    instruction_pointer: i64,
}

impl Processor {
    pub fn new(id: i64) -> Processor {
        ...
        processor.registers.insert(String::from("p"), id);

        processor
    }

    pub fn send_network_tx(&mut self, network_tx: &mut VecDeque<i64>) {
        self.network_rx.append(network_tx);
    }
    ...
    fn execute_rcv(&mut self, a: Argument) -> bool {
        let rx_val = self.network_rx.pop_front();

        let rx_val = match rx_val {
            Some(val) => val,
            None => return false,
        };

        match a {
            Argument::Register(val) => self.set_register_value(val, rx_val),
            Argument::Literal(_) => panic!("Executing rcv instruction on a literal."),
        }
    ...
    pub fn run_program(&mut self) -> VecDeque<i64> {

        let mut network_tx = VecDeque::new();
        ...
        loop {
            ...
            let instr = self.program[self.instruction_pointer as usize].clone();

            match instr {
                Instruction::Snd(a)    => network_tx.push_back(self.execute_snd(a)),
                ...
                Instruction::Jgz(a, b) => {
                    self.instruction_pointer += self.execute_jgz(a, b);
