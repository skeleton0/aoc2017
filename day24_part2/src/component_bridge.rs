...
impl ComponentTree {
    ...
    pub fn find_longest_bridge(&self, node: usize, current_bridge: &Vec<usize>) -> Vec<Vec<usize>> {
        let node = &self.nodes[node];

        let mut current_bridge = current_bridge.clone();
        current_bridge.push(node.component_index);

        let mut longest_bridges = Vec::new();
        longest_bridges.push(current_bridge.clone());

        for child in node.children_indices.iter() {
            let mut bridges = self.find_longest_bridge(*child, &current_bridge);

            if bridges[0].len() > longest_bridges[0].len() {
                longest_bridges = bridges;
            } else if bridges[0].len() == longest_bridges[0].len() {
                longest_bridges.append(&mut bridges);
            }  
        }
